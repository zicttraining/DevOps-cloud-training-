...
# resource "aws_acm_certificate" "bme-dev_cert" {
#   domain_name       = "bmeevent.com"
#   validation_method = "DNS"
#   tags = { Name = "bme-dev-app-cert" }
# }
...
#   for_each = {
#     for dvo in aws_acm_certificate.bme-dev_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "bme-dev_cert_validation" {
#   certificate_arn         = aws_acm_certificate.bme-dev_cert.arn
...
resource "aws_instance" "bme-dev" {
  ...
  tags = { Name = "bme-dev-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "bme-dev_lb" {
  name               = "bme-dev-app-lb"
  internal           = false
  ...
  tags = { Name = "bme-dev-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "bme-dev_tg" {
  name        = "bme-dev-app-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.bme-dev.id
  ...
  tags = { Name = "bme-dev-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "bme-dev_tg_attachment" {
  target_group_arn = aws_lb_target_group.bme-dev_tg.arn
  target_id        = aws_instance.bme-dev.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "bme-dev_http_listener" {
  load_balancer_arn = aws_lb.bme-dev_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "bme-dev_https_listener" {
#   load_balancer_arn = aws_lb.bme-dev_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "bme-dev_record" {
  zone_id = aws_route53_zone.bmeevent_zone.zone_id
  name    = "bmeevent.com"
  type    = "A"
  alias {
    name                   = aws_lb.bme-dev_lb.dns_name
    zone_id                = aws_lb.bme-dev_lb.zone_id
