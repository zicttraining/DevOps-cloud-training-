...
resource "aws_db_subnet_group" "ssl-dev_subnet_group" {
  name       = "ssl-dev-app-db-subnet-group"
...
resource "aws_security_group" "ssl-dev_db_sg" {
  name   = "ssl-dev-app-db-sg"
  vpc_id = aws_vpc.ssl-dev.id # Corrected VPC reference
...
resource "aws_db_instance" "ssl-dev_db" {
  allocated_storage      = 20
  storage_type           = "gp2"
  engine                 = "mysql" # Change to your desired database engine
  engine_version         = "8.0"   # Specify the desired version
  instance_class         = "db.t3.micro"
  db_name                = "ssldevdb"  # Corrected to use db_name
  username               = "admin"       # Username for the database
  password               = "password123" # Replace with a secure password
  db_subnet_group_name   = aws_db_subnet_group.ssl-dev_subnet_group.name
  vpc_security_group_ids = [aws_security_group.ssl-dev_db_sg.id] # Reference to the security group for RDS
