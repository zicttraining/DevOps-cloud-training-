...
resource "aws_s3_bucket_ownership_controls" "bs101-uat_bucket_ownership_controls" {
  bucket = aws_s3_bucket.bs101-uat_bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.bs101-uat_bucket.arn}",
          "${aws_s3_bucket.bs101-uat_bucket.arn}/*"
        ]
      }
    ]
...
resource "aws_s3_bucket_policy" "bs101-uat_bucket_policy" {
  bucket = aws_s3_bucket.bs101-uat_bucket.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow CloudTrail to write logs to this bucket
      {
        ...
        Action   = "s3:PutObject",
        Resource = "${aws_s3_bucket.bs101-uat_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
...
resource "aws_ebs_volume" "mc-uat-app_web_ebs" {
  availability_zone = "us-east-1a" # Corrected spelling
  size              = 10           # Size in GB
  tags = {
    Name = "mc-uat-app-web-ebs"
  }
}
