...
resource "aws_db_subnet_group" "bme-dev_subnet_group" {
  name       = "bme-dev-app-db-subnet-group"
...
resource "aws_security_group" "bme-dev_db_sg" {
  name   = "bme-dev-app-db-sg"
  vpc_id = aws_vpc.bme-dev.id # Corrected VPC reference
...
resource "aws_db_instance" "bme-dev_db" {
  allocated_storage      = 20
  storage_type           = "gp2"
  engine                 = "mysql" # Change to your desired database engine
  engine_version         = "8.0"   # Specify the desired version
  instance_class         = "db.t3.micro"
  db_name                = "bmedevdb"  # Corrected to use db_name
  username               = "admin"       # Username for the database
  password               = "password123" # Replace with a secure password
  db_subnet_group_name   = aws_db_subnet_group.bme-dev_subnet_group.name
  vpc_security_group_ids = [aws_security_group.bme-dev_db_sg.id] # Reference to the security group for RDS
