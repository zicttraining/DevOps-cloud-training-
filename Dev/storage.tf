...
# S3 Bucket for application storage with private access
resource "aws_s3_bucket" "bme-dev_bucket" {
...
resource "aws_s3_bucket_ownership_controls" "bme-dev_bucket_ownership_controls" {
  bucket = aws_s3_bucket.bme-dev_bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.bme-dev_bucket.arn}",
          "${aws_s3_bucket.bme-dev_bucket.arn}/*"
        ]
      }
    ]
...
resource "aws_s3_bucket_policy" "bme-dev_bucket_policy" {
  bucket = aws_s3_bucket.bme-dev_bucket.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow CloudTrail to write logs to this bucket
      {
        ...
        Action   = "s3:PutObject",
        Resource = "${aws_s3_bucket.bme-dev_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
