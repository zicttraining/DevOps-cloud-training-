...
resource "aws_db_subnet_group" "mrmike-prod_app_subnet_group" {
  name       = "mrmike-prod-app-db-subnet-group"
...
resource "aws_security_group" "mrmike-prod_app_db_sg" {
  name   = "mrmike-prod-app-db-sg"
  vpc_id = aws_vpc.mrmike-prod_app.id  # Corrected VPC reference
...
resource "aws_db_instance" "mrmike-prod_app_db" {
  ...
  db_subnet_group_name    = aws_db_subnet_group.mrmike-prod_app_subnet_group.name
  vpc_security_group_ids  = [aws_security_group.mrmike-prod_app_db_sg.id]  # Reference to the security group for RDS

  skip_final_snapshot = true

  tags = {
    Name = "mrmike-prod_app_db"
  }
}
