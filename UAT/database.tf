...
resource "aws_db_subnet_group" "bme-uat_subnet_group" {
...
resource "aws_security_group" "bme-uat_db_sg" {
  name   = "bme-uat-app-db-sg"
  vpc_id = aws_vpc.bme-uat.id # Corrected VPC reference
...
resource "aws_db_instance" "bme-uat_db" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.bme-uat_subnet_group.name
  vpc_security_group_ids = [aws_security_group.bme-uat_db_sg.id] # Reference to the security group for RDS
