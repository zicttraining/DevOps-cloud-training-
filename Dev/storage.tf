...
# S3 Bucket for application storage with private access
resource "aws_s3_bucket" "mc-dev_bucket" {
  bucket = "mc-dev-app-logs-${random_id.bucket_suffix.hex}"

  tags = {
    Name = "mc-uay-app-logs"
  }
}

# Enforce ownership controls on the bucket
resource "aws_s3_bucket_ownership_controls" "mc-dev_bucket_ownership_controls" {
  bucket = aws_s3_bucket.mc-dev_bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.mc-dev_bucket.arn}",
          "${aws_s3_bucket.mc-dev_bucket.arn}/*"
        ]
      }
    ]
...
resource "aws_s3_bucket_policy" "mc-dev_bucket_policy" {
  bucket = aws_s3_bucket.mc-dev_bucket.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow CloudTrail to write logs to this bucket
      {
        ...
        Action   = "s3:PutObject",
        Resource = "${aws_s3_bucket.mc-dev_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
...
resource "aws_ebs_volume" "mc-dev-app_web_ebs" {
  availability_zone = "us-east-1a" # Corrected spelling
  size              = 10           # Size in GB
  tags = {
    Name = "mc-dev-app-web-ebs"
  }
}
