...
# resource "aws_acm_certificate" "mrmike-prod_app_cert" {
#   domain_name       = "bmeevent.com"
#   validation_method = "DNS"
#   tags = { Name = "mrmike-prod-app-cert" }
# }
...
#   for_each = {
#     for dvo in aws_acm_certificate.mrmike-prod_app_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "mrmike-prod_app_cert_validation" {
#   certificate_arn         = aws_acm_certificate.mrmike-prod_app_cert.arn
...
resource "aws_instance" "mrmike-prod_app" {
  ...
  tags = { Name = "mrmike-prod-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "mrmike-prod_app_lb" {
  name               = "mrmike-prod-app-lb"
  internal           = false
  load_balancer_type = "application"
  security_groups    = [aws_security_group.vpc_web_sg.id]
  subnets            = [aws_subnet.public_subnet_1.id, aws_subnet.public_subnet_2.id]

  tags = { Name = "mrmike-prod-app-lb" }
}

# Target Group pointing to the EC2 instance
resource "aws_lb_target_group" "mrmike-prod_app_tg" {
  name        = "mrmike-prod-app-tg"
  port        = 80
  protocol    = "HTTP"
  ...
  tags = { Name = "mrmike-prod-app-tg" }
}

# Register the EC2 instance with the Target Group
resource "aws_lb_target_group_attachment" "mrmike-prod_app_tg_attachment" {
  target_group_arn = aws_lb_target_group.mrmike-prod_app_tg.arn
  target_id        = aws_instance.mrmike-prod_app.id
  port             = 80
}

# HTTP Listener to redirect traffic to HTTPS
resource "aws_lb_listener" "mrmike-prod_app_http_listener" {
  load_balancer_arn = aws_lb.mrmike-prod_app_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "mrmike-prod_app_https_listener" {
#   load_balancer_arn = aws_lb.mrmike-prod_app_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "mrmike-prod_app_record" {
  zone_id = aws_route53_zone.bmeevent_zone.zone_id
  name    = "bmeevent.com"
  type    = "A"
  alias {
    name                   = aws_lb.mrmike-prod_app_lb.dns_name
    zone_id                = aws_lb.mrmike-prod_app_lb.zone_id
