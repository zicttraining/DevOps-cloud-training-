...
resource "aws_security_group" "bme_uat_app_db_sg" {
  name   = "bme-uat-app-db-sg"
  vpc_id = aws_vpc.bme_uat_app.id  # Corrected VPC reference
...
resource "aws_db_instance" "bme_uat_app_db" {
  ...
  db_subnet_group_name    = aws_db_subnet_group.bme_uat_app_subnet_group.name
  vpc_security_group_ids  = [aws_security_group.bme_uat_app_db_sg.id]  # Reference to the security group for RDS
