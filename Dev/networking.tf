# ACM Certificate for HTTPS with domain MCevent.com
# Uncomment the following block and ensure ACM certificate validation is set up correctly if you want to use HTTPS.

# resource "aws_acm_certificate" "MC-dev_cert" {
#   domain_name       = "MCevent.com"
#   validation_method = "DNS"
#   tags = { Name = "MC-dev-app-cert" }
# }
...
#   for_each = {
#     for dvo in aws_acm_certificate.MC-dev_cert.domain_validation_options : dvo.domain_name => {
#       name   = dvo.resource_record_name
#       type   = dvo.resource_record_type
#       record = dvo.resource_record_value
#     }
#   }
#   zone_id = aws_route53_zone.MCevent_zone.zone_id
...
# resource "aws_acm_certificate_validation" "MC-dev_cert_validation" {
#   certificate_arn         = aws_acm_certificate.MC-dev_cert.arn
#   validation_record_fqdns = [for record in aws_route53_record.cert_validation : record.fqdn]
# }

# Hosted Zone for the domain MCevent.com
resource "aws_route53_zone" "MCevent_zone" {
  name = "MCevent.com"
}

# Public EC2 Instance in a Public Subnet
resource "aws_instance" "MC-dev" {
  ...
  tags = { Name = "MC-dev-app-server" }
}

# Load Balancer for HTTP and HTTPS traffic
resource "aws_lb" "MC-dev_lb" {
  name               = "MC-dev-app-lb"
  internal           = false
...
# resource "aws_lb_listener" "MC-dev_https_listener" {
#   load_balancer_arn = aws_lb.MC-dev_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "MC-dev_record" {
  zone_id = aws_route53_zone.MCevent_zone.zone_id
  name    = "MCevent.com"
  type    = "A"
  alias {
    name                   = aws_lb.MC-dev_lb.dns_name
    zone_id                = aws_lb.MC-dev_lb.zone_id
